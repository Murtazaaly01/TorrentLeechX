...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(
        f"<a href='tg://user?id={g_id}'>➩</a> Now Uploading to  Cloud!!!"
    )
    ...
    if os.path.isfile(file_upload):
        ...
        button = [[pyrogram.InlineKeyboardButton(text="CloudUrl ", url=f"{gauti}")]]
        if INDEX_LINK:
        ...
        LOGGER.info(gjay)
        button = [[pyrogram.InlineKeyboardButton(text="CloudUrl", url=f"{gautii}")]]
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{os.path.basename(file_upload)}/"
            tam_link = requests.utils.requote_uri(indexurl)
            LOGGER.info(tam_link)
